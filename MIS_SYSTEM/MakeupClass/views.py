...
def MakeupClass(request,id):
    ...
    if request.method == 'POST':
        ...
        if(request.POST.get('options') == "others"):
            ...
            historyback.save()
            memopk=Memo_Routing.objects.get(id = memo_makeupclass.pk)
            if (data.Status_Dept == "Chairman"):
                Memo_Routing.objects.filter(id = memopk).update(Date_Chairman_Approved=datetime.today())
            if (data.Status_Dept == "Dean"):
                Memo_Routing.objects.filter(id = memopk).update(Date_Chairman_Approved=datetime.today(),Date_Dean_Approved=datetime.today())
