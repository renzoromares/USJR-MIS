...
def RequestCertificate(request,id):
    ...
    if request.method == "POST":
        ...
        certificate.save()
        memo_certificate = Memo_Routing(Id_Number = employeeID, Type_Request = 'Request for Certificate', Date_Faculty_Submitted = datetime.today().strftime('%Y-%m-%d'), FormID = formPK)
        memo_certificate.save()
        history = TransacHistory(Id_Number = employeeID, Transac_Type = 'Certificate', Type='Submitted', Date = datetime.today().strftime('%Y-%m-%d'))
        history.save()
