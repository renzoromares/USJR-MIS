...
def RoomTransferPermanent(request,id):
    ...
    if request.method == "POST":
        form = Form(Id_Number=employeeID, Type='RoomTransfer(Permanent)',Date_Requested=datetime.today().strftime('%Y-%m-%d,%H:%M'),Date_Approved=None,Status='Pending')
        form.save()
        formPK = Form.objects.get(Form_ID = form.pk)
        roomTransfer = Room_Transfer(Id_Number=employeeID, Type='RoomTransfer(Permanent)', Date_Notify=datetime.today().strftime('%Y-%m-%d,%H:%M'), Subject=request.POST["Subject"], Offer_Code=request.POST["Offer_Code"], Time_Day=request.POST["Time_Day"], Room_From=request.POST["Room_From"], Room_To=request.POST["Room_To"],Date_Effective=None ,Reason=request.POST["Reason"], FormID = formPK)
        roomTransfer.save()
        memo_roomTransfer = Memo_Routing(Id_Number=employeeID, Type_Request = 'RoomTransfer(Permanent)', Date_Faculty_Submitted = datetime.today().strftime('%Y-%m-%d,%H:%M'), FormID = formPK)
        memo_roomTransfer.save()
        history = TransacHistory(Id_Number = employeeID, Transac_Type = 'RoomTransfer(Permanent)', Type='Submitted', Date = datetime.today().strftime('%Y-%m-%d,%H:%M'))
        history.save()
