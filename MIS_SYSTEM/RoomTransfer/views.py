...
def RoomTransferPermanent(request,id):
    ...
    if request.method == "POST":
        ...
        roomTransfer.save()
        memo_roomTransfer = Memo_Routing(Id_Number=employeeID, Type_Request = 'RoomTransfer(Permanent)', Date_Faculty_Submitted = datetime.today().strftime('%Y-%m-%d'), FormID = formPK)
        memo_roomTransfer.save()
        history = TransacHistory(Id_Number = employeeID, Transac_Type = 'RoomTransfer(Permanent)', Type='Submitted', Date = datetime.today().strftime('%Y-%m-%d'))
        history.save()
...
def RoomTransferTemporary(request,id):
    ...
    if request.method == "POST":
        ...
        roomTransfer.save()
        memo_roomTransfer = Memo_Routing(Id_Number=employeeID, Type_Request = 'RoomTransfer(Temporary)', Date_Faculty_Submitted = datetime.today().strftime('%Y-%m-%d'), FormID = formPK)
        memo_roomTransfer.save()
        history = TransacHistory(Id_Number = employeeID, Transac_Type = 'RoomTransfer(Temporary)', Type='Submitted', Date = datetime.today().strftime('%Y-%m-%d'))
        history.save()
